...
class Level {
 ...
 public:
  ...
  explicit Level(const std::array<uint8_t, n_tiles> &data) : tiles_{data} {
    for (int y = 0; y < height; ++y) {
      for (int x = 0; x < width; ++x) {
        ...
        } else if (tiles_[x + y * width] == std::uint8_t(TileType::artillery)) {
          artillery_.push_back(x + y * width);
        }
      }
    }
  }
  ...
  [[maybe_unused]] auto &artillery() { return artillery_; }
  [[maybe_unused]] [[nodiscard]] const auto &artillery() const { return artillery_; }
};
